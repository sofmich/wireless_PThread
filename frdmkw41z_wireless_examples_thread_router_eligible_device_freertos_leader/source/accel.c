...
uint8_t Get_Accel(uint8_t* Data){
	int16_t xRawData, yRawData,zRawData;
	uint8_t i = 0, n = 0;
	...
	zRawData = (int16_t)((uint16_t)((uint16_t)sensorData.accelZMSB << 8) | (uint16_t)sensorData.accelZLSB) / 4U;
	...
	sprintf(zDataString, "%d",(int)zRawData);
	n = 0;
