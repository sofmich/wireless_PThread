...
{
    uint8_t pAccelString[120];
    uint32_t ackPloadSize = 0;

    if(gCoapGET_c == pSession->code)
    {
    	/* Get Accel Values as a string saved to payload*/
		uint8_t i = Get_Accel(pAccelString);
		char address[INET6_ADDRSTRLEN];

		/*Get the address*/
		ntop(AF_INET6, (ipAddr_t*)&pSession->localAddr.addr8, address, INET6_ADDRSTRLEN);

		uint8_t n = 0;
		while(address[n] != 0) n++;

		for(uint8_t j = 0; j <  n; j++, i++){ /*Address length to payload size at coap send function*/
			pAccelString[i] = address[j];
		}

		ackPloadSize = (uint32_t)i;
        if(gCoapConfirmable_c == pSession->msgType)
        {
        	if (gCoapFailure_c!=sessionStatus)
        	{
        		COAP_Send(pSession, gCoapMsgTypeAckSuccessContent_c, pAccelString, ackPloadSize);
        	}
        }
        else
        {
